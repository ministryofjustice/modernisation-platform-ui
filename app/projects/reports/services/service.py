...
def get_all_json_data(org, repo, branch, directory):
    ...
    try:
        with open(CACHE_FILE, "w") as f:
            json.dump({"data": json_data_list, "timestamp": now}, f)
    except IOError:
        pass
...
def get_readme_incident_info(org, repo, branch, app_names):
    ...
    try:
        with open(README_CACHE_FILE, "w") as f:
            json.dump({"data": incident_info, "timestamp": now}, f)
    except IOError:
        pass
    ...
    for path in readme_paths:
        try:
            ...
            if response.status_code == 200:
                ...
                logger.debug(
                    f"README found for {app_name}: hours={bool(incident_hours)}, contact={bool(incident_contact)}"
                )
